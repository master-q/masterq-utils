...
objdir="/home/kiwamu/src/netbsd/current/src/obj/"
cmd_disklabel="$objdir/tooldir.Linux-2.6.38-2-amd64-x86_64/bin/nbdisklabel-i386"
cmd_installboot="$objdir/tooldir.Linux-2.6.38-2-amd64-x86_64/bin/nbinstallboot"
cmd_makefs="$objdir/tooldir.Linux-2.6.38-2-amd64-x86_64/bin/nbmakefs"
cmd_stat="$objdir/tooldir.Linux-2.6.38-2-amd64-x86_64/bin/nbstat"
file_bootxx_ffsv1="$objdir/destdir.i386/usr/mdec/bootxx_ffsv1"
file_mdec_boot="$objdir/destdir.i386/usr/mdec/boot"
...
cp ${file_mdec_boot} "$dst" || die "couldn't copy /usr/mdec/boot"

echo " => building filesystem image"
${cmd_makefs} "$img" "$dst" || die "couldn't build filesystem image"

echo " => installing bootxx_ffsv1"
${cmd_installboot} -m i386 "$img" ${file_bootxx_ffsv1} || die "couldn't install bootxx_ffsv1"

echo " => writing disklabel"
imgsize=$(${cmd_stat} -L -f %z "${img}")
echo "$disklabel" | \
...
${cmd_disklabel} -R -F "${img}" "${dstbase}/disklabel" || die "couldn't write disklabel"
